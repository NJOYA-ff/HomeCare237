...
const AdminSettings: React.FC = () => {
  ...
  const [newRegistrationAlert, setNewRegistrationAlert] = useState(
    () => localStorage.getItem("hc_admin_new_reg_alert") !== "false"
  );
  const [analyticsDigest, setAnalyticsDigest] = useState(
    () => localStorage.getItem("hc_admin_analytics_digest") === "true"
  );
  const [systemAlerts, setSystemAlerts] = useState(
    () => localStorage.getItem("hc_admin_system_alerts") !== "false"
  );
  ...
  return (
    <IonPage>
      ...
      <IonContent className="settings-content">
        ...
        <IonListHeader className="settings-section-header">System Alerts</IonListHeader>
        <IonList className="settings-list">
          <IonItem>
            <IonIcon icon={peopleOutline} slot="start" className="settings-icon" />
            <IonLabel>
              <h3>New User Registrations</h3>
              <p className="settings-desc">Alert when a new patient or doctor registers</p>
            </IonLabel>
            <IonToggle
              slot="end"
              checked={newRegistrationAlert}
              onIonChange={(e) => {
                setNewRegistrationAlert(e.detail.checked);
                localStorage.setItem("hc_admin_new_reg_alert", String(e.detail.checked));
              }}
            />
          </IonItem>
          <IonItem>
            <IonIcon icon={statsChartOutline} slot="start" className="settings-icon" />
            <IonLabel>
              <h3>Daily Analytics Digest</h3>
              <p className="settings-desc">Receive a daily summary of platform activity</p>
            </IonLabel>
            <IonToggle
              slot="end"
              checked={analyticsDigest}
              onIonChange={(e) => {
                setAnalyticsDigest(e.detail.checked);
                localStorage.setItem("hc_admin_analytics_digest", String(e.detail.checked));
              }}
            />
          </IonItem>
          <IonItem>
            <IonIcon icon={alertCircleOutline} slot="start" className="settings-icon" />
            <IonLabel>
              <h3>System Alerts</h3>
              <p className="settings-desc">Critical system and error notifications</p>
            </IonLabel>
            <IonToggle
              slot="end"
              checked={systemAlerts}
              onIonChange={(e) => {
                setSystemAlerts(e.detail.checked);
                localStorage.setItem("hc_admin_system_alerts", String(e.detail.checked));
              }}
            />
          </IonItem>
        </IonList>
